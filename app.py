...
app = Flask(__name__)
##postgress
engine = create_engine("mysql+pymysql://root:345678@localhost:3306/siraj")
...
def view():
    if request.method == "POST":
        ...
        db.execute("INSERT into fee(fisrtname, lastname, session, submittedfee, duefee, totalfee) VALUES (:fisrtname, :lastname, :session, :submittedfee, :duefee, :totalfee)",
                {"fisrtname": fname, "lastname": lname, "session": session, "submittedfee": sub_fee, "duefee": due_fee, "totalfee": tot_fee})
        db.commit()
